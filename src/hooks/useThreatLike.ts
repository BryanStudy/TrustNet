...
export function useThreatLike({
  ...
  const {
    ...
    queryFn: async () => {
      const res = await axios.get(`/digital-threats/${threatId}/like-status`);
  ...
  const {
    ...
    mutationFn: async () => {
      await axios.post(`/digital-threats/${threatId}/like`, {
        createdAt,
      });
    },
    onSuccess: () => {
  ...
  const {
    ...
    mutationFn: async () => {
      await axios.delete(`/digital-threats/${threatId}/like`, {
        data: { createdAt },
      });
    },
    onSuccess: () => {
