...
import { PutCommand, QueryCommand, ScanCommand, GetCommand, UpdateCommand, DeleteCommand, BatchWriteCommand } from '@aws-sdk/lib-dynamodb';
...
async function handleDeleteUser(event) {
  try {
    ...
    for (const threat of threats) {
      ...
      const { Items: threatLikes = [] } = await ddbDocClient.send(threatLikesQuery);
...
export const handler = async (event) => {
  try {
    switch (event.routeKey) {
      ...
      case 'POST /users/admin-create':
        return await handleAdminCreateUser(event);
      case 'GET /users/{id}':
      ...
      case 'OPTIONS /users':
      case 'OPTIONS /users/admin-create':
